    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex-1 flex flex-col">
      ...
      <div className="pt-8">
        {user ? (
          ...
          <Link
            href={`/${tenantSlug}/sign-in?redirect_url=/${tenantSlug}/book?step=review&serviceId=${serviceId}&branchId=${branchId}&date=${date}&time=${time}${campaignCode ? `&c=${campaignCode}` : ""}`}
      ...
      <Link href={`/${tenantSlug}/book?step=time&serviceId=${serviceId}&branchId=${branchId}${campaignCode ? `&c=${campaignCode}` : ""}`} className="inline-flex items-center text-sm font-bold text-muted-foreground hover:text-primary transition-colors uppercase tracking-widest gap-2">
        ← Back to times
      </Link>
    </div>
  );
